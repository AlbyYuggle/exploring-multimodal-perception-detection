...
def predict_image(model, image, dimg, image_name):
    """
    ...
    #dimg = np.expand_dims(dimg, axis=-1)
    #img = np.concatenate((img, dimg), axis=2)
    img = transform(img).type(torch.FloatTensor)
    #print(img.shape)
    #print(dimg.shape)
    dimg = transform(dimg).type(torch.FloatTensor)
    #print(dimg.shape)   
    #print(img.shape, dimg.shape)

    with torch.no_grad():
        img = Variable(img[None, :, :, :])
        img = img.cuda()
        dimg = Variable(dimg[None, :, :, :])
        dimg = dimg.cuda()

        pred = model(img, dimg)  # 1xSxSx(B*5+C)
        pred = pred.cpu()
