...
class ResNet(nn.Module):
    def __init__(self, block, layers):
        ...
        self.layer1 = self._make_layer(block, 32, layers[0], False)


        self.conv1y = nn.Conv2d(3, 32, kernel_size=7, stride=2, padding=3, bias=False)
        self.bn1y = nn.BatchNorm2d(32)
        self.reluy = nn.ReLU(inplace=True)
        self.maxpooly = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1y = self._make_layer(block, 32, layers[0], True)
        ...
        self.conv_end = nn.Conv2d(
            256, 13, kernel_size=3, stride=1, padding=1, bias=False
        )
    ...
    def _make_layer(self, block, planes, blocks, stride=1, flag=None):
        downsample = None
...
def update_state_dict(pretrained_model, model):
    ...
    for k in new_state_dict.keys():
        if k in dd.keys() and not k.startswith("fc"):
            dd2 = model.state_dict()
            try:
                dd2[k] = new_state_dict[k]
                model.load_state_dict(dd2, strict=False)
...
def resnet50(pretrained=False, **kwargs):
    ...
    """
    model = ResNet(Bottleneck, Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
